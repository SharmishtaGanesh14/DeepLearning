...
class ImageCaptionLSTM(nn.Module):
    def __init__(self, img_feat_size, embed_size, hidden_size, vocab_size, embedding_matrix):
        ...
        self.cell_init = nn.Linear(img_feat_size, hidden_size)    # init cell from image
    ...
    def forward(self, img_feat, max_len=20):
        ...
        # Initialize hidden and cell state from image
        h0 = torch.tanh(self.hidden_init(img_feat)).unsqueeze(0)  # [1, batch, hidden_size]
        c0 = torch.tanh(self.cell_init(img_feat)).unsqueeze(0)    # [1, batch, hidden_size]
        hidden = (h0, c0)
        ...
        for _ in range(max_len):
            out, hidden = self.lstm(input_t, hidden)       # [batch, 1, hidden_size]
