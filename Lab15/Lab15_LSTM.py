...
class ImageCaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_feat = torch.tensor(np.fromstring(row["features"], sep=' '), dtype=torch.float32)
        cap_embed = torch.tensor(np.fromstring(row["embedding"], sep=' '), dtype=torch.float32)
...
class ImageCaptionlstm(nn.Module):
    ...
    def forward(self, img_feats):
        x = self.img_fc(img_feats).unsqueeze(1)   # [batch, seq_len=1, embed_size]
        hiddens, _ = self.lstm(x)                  # [batch, 1, hidden_size]
        out = self.linear(hiddens)                # [batch, 1, embed_size]
        return out.squeeze(1)                     # [batch, embed_size]

def main():
    # Load data
    data = pd.read_csv("DataAndProcessing/ground_truth.csv")
    features = pd.read_csv("DataAndProcessing/features_to_rnn.csv")
    ...
    model = ImageCaptionlstm().to(device)
